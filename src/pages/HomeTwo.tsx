...
function decideVariant(num: number) {
    if (num % 3 === 0) {return 'three'}
    else if (num % 2 ===0) {return 'two'}
    return 'one'
}

export default function HomeTwo() {
    const multiply = Array.from(Array(50).keys())
    return (
        <main className='h-screen w-full'>
        {/* {
            multiply.map((_, idx) => {
                return <LogosSlide key={idx} variant={decideVariant(idx)} />
            })
        } */}
        <div className="flex h-screen relative z-10">
            ...
            <div className={` text-dark w-10/12 bg-white/90 dark:bg-dark/95 dark:text-light pt-10 lg:pt-0 absolute z-20 left-1/2 top-0 -translate-x-1/2 translate-y-[10%] rounded-3xl p-6 md:translate-y-[3%]`}>
